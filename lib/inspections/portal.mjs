...
// `zips`, when present, narrows a jurisdiction to those postal codes. Henrico
// is county-wide and reaches well past the parts of it worth reading about, so
// it's limited to the west-end / near-west ZIPs; Richmond City is small enough
// to take whole.
export const JURISDICTIONS = [
  {
    ...
    path: 'va-henrico',
    name: 'Henrico County',
    zips: ['23238', '23229', '23226', '23230', '23228', '23294', '23060', '23233'],
  },
...
export async function fetchInspections(jurisdiction, startDate, endDate, { maxPages = 40 } = {}) {
  ...
  for (let page = 0; page < maxPages; page++) {
    ...
    rows.push(...fresh);
    // Page on the unfiltered count — the ZIP filter is applied afterwards, so
    // a page that's entirely out-of-area still means there's more to read.
    if (list.length < PAGE_SIZE || !fresh.length) break;
  }
  return rows.filter((row) => inArea(row, jurisdiction)).map((row) => normalize(row, jurisdiction));
}

// The portal has no ZIP filter of its own, so it's applied here. Some rows carry
// a ZIP+4 ("23059-6501"), hence matching on the five-digit prefix.
const zip5 = (value) => (String(value ?? '').match(/\d{5}/) || [''])[0];

function inArea(row, jurisdiction) {
  if (!jurisdiction.zips?.length) return true;
  return jurisdiction.zips.includes(zip5(row.zip));
}
...
function normalize(row, jurisdiction) {
  ...
  const cityLine = [[row.city, row.state].filter(Boolean).join(', '), zip].filter(Boolean).join(' ');
